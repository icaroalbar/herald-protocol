...
— expõe `GET /metrics` (via `@heraldserver/gateway`) em formato de texto Prometheus/OpenMetrics
em vez do JSON default do `InMemoryMetricsCollector`.

Pacote opcional, separado — `@heraldserver/sdk` e `@heraldserver/gateway` não dependem de
...
`InMemoryMetricsCollector` (o default, em `@heraldserver/sdk`) agrega por `agentId` — cabe bem
...
individual (ainda disponível via `outpost_reports` no `@heraldserver/server`, se precisar),
